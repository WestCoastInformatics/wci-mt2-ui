...
export class CodeUtility {
	 ...
	 */
	static addIfNotEmpty(originalString: string, stringToAdd: string, addToEnd = true, addOr = false): string {
	...
	static removeFinal(str: string, charsToRemove: string): string {
		...
		const regex = new RegExp(`${charsToRemove}\s*$`);
	 ...
	 */
	static hasValue(variable, rejectEmpty = true, rejectNullStringValues = false) {
		if (
			variable == undefined ||
			variable == null ||
	...
	static parseJsonDate(jsonDate) {
		if (this.hasValue(jsonDate)) {
			const date = moment.unix(jsonDate / 1000);
	...
	static formatJsonDate(jsonDate: string, format: string = this.DATE_FORMAT_REVERSE) {
		const date = this.parseJsonDate(jsonDate);
	...
	static compareDates(firstDate, secondDate, format = this.DATE_FORMAT_US_STANDARD_WITH_24_HOUR_TIME) {
		const momentDateA = moment(firstDate, format);
		const momentDateB = moment(secondDate, format);
	...
	static getUrlString(params, keys = [], isArray = false) {
		//const b = {a: 1, b: {c: [1, 2], d: 'property'}};

		const urlString = Object.keys(params)
			.map((key) => {
				// get the param value
				...
				if ('[object Object]' === Object.prototype.toString.call(value) || Array.isArray(value)) {
					...
					if (keys.length > 0) {
						...
						transfomedKey = allKeys.reduce((keyName, i) => {
	...
	static getParamsAsObject(query) {
		...
		while ((expression = regex.exec(query))) {
			...
			const value = decode(expression[2]);
