...
export class RefsetUtility {
	static mockedVersionOptions = [
		{ value: '2021-02-21', display: 'In Development' },
		{ value: '2021-01-15', display: 'Published (2021-01-15)' },
		{ value: '2020-11-23', display: 'Beta (2020-11-23)' },
	];
	...
	static getVersionOptions(refset, valueField = 'id') {
		let versionOptions = [];

		for (const version of refset.versionList) {
			...
			const option: any = { value: value, display: version.date + ' (' + displayStatus + ')', date: version.date, versionDate: versionDate, status: displayStatus };
	...
	static sortDescriptions(descriptions, fullyQualifiedLanguageRefsets) {
		...
		return descriptions.sort((description1, description2) => {
			const languageCompareValue = languagePriority[description1.language] - languagePriority[description2.language];
			...
			const typeCompareValue = typePriority[description1.type] - typePriority[description2.type];
			...
			const descriptionCompareValue = description1.toString().localeCompare(description2);

			return descriptionCompareValue;
		});
	}

	static addRemoveMembersByList(refsetInternalId: string, refsetId: string, listOfIds: any, operation: string, callback: any, notificationService: any, refsetService: any, router: any): void {
		if (!listOfIds?.length) {
			return;
		}

		let operationFunction: any;
		let messageModifier = '';
