		...
		this.refsetService.getDiscussionThreads(this.type, this.refsetInternalId, this.conceptId).subscribe({
			next: (results) => {
				...
				results.items = results.items.filter(
					(r) => !r.posts[0].privatePost || r.posts[0].user.userName === this.user.userName || this.roles?.includes('ADMIN') || this.user?.roles?.includes('all-all-admin')
				);
	...
	onGridCellClick = (event) => {
		for (const thread of this.threadsData) {
			if (thread.id === event.data.id) {
				...
				this.selectedThread.posts = this.selectedThread.posts.filter(
					(p) => !p.privatePost || p.user.userName === this.user.userName || this.roles?.includes('ADMIN') || this.user?.roles?.includes('all-all-admin')
				);
			}
		}
