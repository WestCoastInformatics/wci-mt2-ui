...
@Component({
	selector: 'app-mapset-new-records',
	templateUrl: './mapset-new-records.component.html',
	styleUrls: ['./mapset-new-records.component.scss'],
})
export class MapsetNewRecordsComponent implements OnInit {
	user: User;
	searchInput = '';
	viewOptions = [
		...
		return {
			...
			getRows: (rowParams) => {
				...
				if (this.isNewPageSize) {
					...
					this.mapSetSubscription = this.refsetService.getNewMappingsByMapset(this.mapsetCode, restParams).subscribe({
						next: (results) => {
