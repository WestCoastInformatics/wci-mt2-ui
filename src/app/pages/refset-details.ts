...
export class RefsetDetailsComponent implements OnInit {
				...
				this.breadcrumbService.setBreadcrumbs([
					{
						path:
							'/organization/' +
							this.refsetData.project.edition.organizationId +
							'/edition/' +
							this.refsetData.project.edition.id +
							'/projects/' +
							this.refsetData.project.id +
							'/refsets',
						label: 'Projects',
					},
	...
	loadRefset(): void {
		this.refsetService.getRefset(this.refsetId, this.versionDate).subscribe({
			next: (results) => {
				...
				if (
					(this.refsetData.versionStatus == Constants.IN_DEVELOPMENT && this.refsetData?.roles?.includes('VIEWER')) ||
				...
				if (this.refsetData.active) {
					...
					this.changeRefsetStatusText =
						'You are about to inactivate this reference set, preventing it from being used in future published versions of the terminiology. You will be able to reactivate it.';
				} else {
	...
	changeConceptDetailsLanguage() {
		this.selectedConceptDetailLanguageIndex = this.languageOptions.findIndex((option) => option.value === this.selectedConceptDetailLanguage);
					...
					...[
						{
							...
							cellClass: 'rt2-details-column-modified-date',
						...
						},
						{
							...
							headerName: '',
							minWidth: 65,
							cellClass: 'rt2-details-column-actions',
							cellRenderer: 'templateRenderer',
							cellRendererParams: {
	...
	changeActiveInactiveStatus(): void {
		const filters = this.membersGridApi.getFilterModel();
	...
	validatePublishDate = (date: string) => {
		const a = CodeUtility.DATE_FORMAT_REVERSE_ONLY_NUMBERS;
		const b = Constants.EXCLUSION;
		const c = UiUtility.getIconImageUrl('test');
		...
		this.refsetService.deleteDevelopmentVersion(this.refsetData.id).subscribe({
			next: (results) => {
				this.notificationService.show(
					'The "In Development" version of reference set "' + this.refsetData.name + '" (' + this.refsetData.refsetId + ') has been deleted. This can not be undone.',
					null,
					'success',
					{
						timeOut: 0,
	...
	loadConceptDetail(concept) {
		...
		this.refsetService.getMembersDetails(concept?.code, { refsetInternalId: this.refsetData.id }).subscribe({
			next: (results) => {
				...
				this.conceptDescriptions = results.descriptions.filter(function (description) {
	...
	showMembersSearchBar(): boolean {
		return (
			(this.showTable && this.membersTableDisplay === 'inline-block' && this.membersTaxonomyDisplay === 'none') ||
	...
	downloadMembersTable() {
		this.membersGridApi.exportDataAsCsv({
			columnKeys: this.membersColumnDefs
				.filter((value) => {
